...
- **优先检查本地 userdata 模板**：调用 list_userdata_templates 查看是否有现成的安装脚本
  - 如果有匹配的模板（如用户要部署 nginx，本地有 nginx-installation-bash），直接调用 exec_userdata 执行，无需手写命令
  - **注意 Min Memory 列**：如果模板标注了最低内存（如 4096MB），必须确保当前场景的实例规格满足要求。如果不满足，先销毁场景，用更大规格的模板重建
  - 如果没有匹配的模板，再用 exec_command 手动执行安装命令
...
7. 新建云主机可能有 cloud-init 占用 apt 锁，如遇 apt lock 错误，等待 30-60 秒后重试
...
4. 调用 compose_up 启动部署（异步，会自动按依赖顺序部署）
5. 部署完成后用 list_cases 查看各服务状态和输出
